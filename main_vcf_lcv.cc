...
int main(int argc, char* argv[])
{
	...
	if (in_file.is_open() && index_file.is_open() && vcf_file.is_open()) {
		...
		for(int i=0; i<NREAD; i++) {
			...
			tmp_global_count = sum_vcf_lcv(in_cache, index_cache, vcf_cache, lcv_cache, cache_pix, temp_total, temp_count, temp_total2, temp_count2, temp_total3, temp_count3);
		...
		if(remainder_pix) {
			...
			tmp_global_count = sum_vcf_lcv(remain_in_cache, remain_index_cache, remain_vcf_cache, remain_lcv_cache, remainder_pix, temp_total, temp_count, temp_total2, temp_count2, temp_total3, temp_count3);
